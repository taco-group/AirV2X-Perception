...
from opencood.visualization import my_vis, simple_vis, vis_utils
...
def test_parser():
    ...
    parser.add_argument(
        "--cavnum", type=int, default=5, help="number of agent in collaboration"
    )
    parser.add_argument(
        "--fix_cavs_box",
        "-f",
    ...
    parser.add_argument(
        "--note", default="[newtest_ego_all_gt]", type=str, help="any other thing?"
    )
...
def main():
    ...
    if "Camera" in hypes["fusion"]["core_method"]:
        ...
        if "/valid_data_info.pkl" in hypes["valid_data"]:
            hypes["valid_data"] = hypes["valid_data"].replace(
                "valid_data_info.pkl", "updated_valid_data_info.pkl"
            )
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if "depth_items" in infer_result and opt.depth_metric:
                ...
                depth_stat.append(
                    inference_utils.depth_metric(
                        depth_items, hypes["fusion"]["args"]["grid_conf"]
                    )
                )

            cavnum = 0
            if opt.fix_cavs_box:
                pred_box_tensor, gt_box_tensor, pred_score, cavnum, cav_id_list = (
                    inference_utils.fix_cavs_box(
                        pred_box_tensor, gt_box_tensor, pred_score, batch_data
                    )
                )
            ...
            if (i % opt.save_vis_interval == 0) and (pred_box_tensor is not None):
                ...
                co_feats = (
                    infer_result["output_dict"]["ego"]["psm"][0].max(dim=0)[0].sigmoid()
                )
                ...
                def vis_confmap(feat, prefix, figsize=(10, 3.5)):
                    ...
                    ax = sns.heatmap(
                        feat.detach().cpu().numpy(), cbar=True, vmin=0, vmax=1
                    )
                    ax.set_xticks([])
                    ax.set_yticks([])
                    fig.tight_layout()
                    plt.savefig(
                        os.path.join(img_save_path_root, "{}.png".format(prefix))
                    )
                    plt.close()
                    return
                ...
                if "psm_single" in infer_result["output_dict"]["ego"]:
                    s_feats = (
                        infer_result["output_dict"]["ego"]["psm_single"][0]
                        .max(dim=0)[0]
                        .sigmoid()
                    )
    ...
    _, ap50, ap70 = eval_utils.eval_final_results(
        result_stat, opt.model_dir, noise_level
    )
    if opt.depth_metric:
        ...
        yaml_utils.save_yaml(
            ...
            os.path.join(opt.model_dir, f"eval_depth_{noise_level}.yaml"),
        )
