...
def test_parser():
    ...
    parser.add_argument(
        "--save_vis", action="store_true", help="whether to save visualization result"
    )
    parser.add_argument(
        "--save_npy",
        action="store_true",
        help="whether to save prediction and gt resultin npy_test file",
    )
    parser.add_argument(
...
def main():
    ...
    # add noise to pose.
    # pos_std_list = [0, 0.2, 0.4, 0.6] # 位姿噪声标准差
    # rot_std_list = [0, 0.2, 0.4, 0.6] # 旋转噪声标准差
    ...
    for use_laplace in use_laplace_options:
        ...
        for pos_mean, pos_std, rot_mean, rot_std in zip(
            ...
            hypes["wild_setting"].update(
                {"loc_err": True, "ryp_std": rot_std, "xyz_std": pos_std}
            )
            ...
            noise_level = (
                f"{pos_std}_{rot_std}_{pos_mean}_{rot_mean}_"
                + opt.fusion_method
                + suffix
                + opt.note
            )
                ...
                with torch.no_grad():
                    ...
                    if opt.save_vis:
                        ...
                        vis_save_path = os.path.join(
                            opt.model_dir, "vis_3d/3d_%05d.png" % i
                        )
                        ...
                        if not os.path.exists(vis_save_path):
                            os.makedirs(vis_save_path)
                        vis_save_path = os.path.join(
                            opt.model_dir, "vis_bev/bev_%05d.png" % i
                        )
                        simple_vis.visualize(
                            pred_box_tensor,
                            gt_box_tensor,
            ...
            ap30, ap50, ap70 = eval_final_results(
                result_stat, opt.model_dir, noise_level, opt.eval_epoch
            )
            AP30.append(ap30)
            AP50.append(ap50)
            AP70.append(ap70)
            ...
            yaml_utils.save_yaml(
                dump_dict,
                os.path.join(opt.model_dir, f"AP030507_{opt.note}{suffix}.yaml"),
            )
...
def eval_final_results(result_stat, save_path, infer_info=None, epoch=None):
    ...
    if infer_info is None:
        ...
        yaml_utils.save_yaml(
            dump_dict, os.path.join(save_path, f"eval_{infer_info}.yaml")
        )
