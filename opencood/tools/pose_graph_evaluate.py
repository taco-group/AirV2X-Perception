...
def vis_data(trans_error_list, rot_error_list, filenames, save_path, std):
    ...
    for error_type in ["trans", "rot"]:
        ...
        # pdf
        sns.displot(
            error_list, kind="kde", bw_adjust=0.1, legend=False, aspect=1.2, linewidth=2
        )
        xtitle = " error (m)" if error_type == "trans" else " error (°)"
        xtitle = error_type + xtitle
        plt.xlabel(xtitle)
        ...
        for error_term in error_list_np:
            error_term[error_term > 1] = (
                1 + np.random.randn(np.sum(error_term > 1)) * 0.05
            )
...
def evaluate_pose_graph(data_dict, save_path, std=0.2):
    ...
    filenames = [
        "correction w/ uncertainty",
        "correction w/o uncertainty",
        "w/o correction",
    ]
    ...
    for sample_idx, content in tqdm(data_dict.items()):
        ...
        pred_corners_list = [
            np.array(corners, dtype=np.float64) for corners in pred_corners_list
        ]
        uncertainty_list = content["uncertainty_np_list"]
        uncertainty_list = [
            np.array(uncertainty, dtype=np.float64) for uncertainty in uncertainty_list
        ]
        ...
        diffs[:, 1:, 2] = np.minimum(
            diffs[:, 1:, 2], 360 - diffs[:, 1:, 2]
        )  # do not include ego
        ...
        if DEBUG:
            if (diffs[0] > 1).any():
                ...
                vis_pose_graph(
                    pose_after, pred_corners_list, save_dir_path=pose_graph_save_dir
                )
                np.savetxt(
                    os.path.join(pose_graph_save_dir, "pose_info.txt"),
                    diffs.reshape(-1, 3),
                    fmt="%.4f",
                )
...
evaluate_json = (
    "/GPFS/rhome/yifanlu/OpenCOOD/stage1_boxes/opv2v/train/stage1_boxes.json"
)
