...
def test_parser():
    ...
    parser.add_argument(
        "--save_pred",
        action="store_true",
        help="whether to save prediction result in pkl file",
    )
    parser.add_argument(
        "--result_file",
        type=str,
...
def main():
    ...
    for scenario in combined_stat:
        ...
        with open(os.path.join(saved_path, opt.result_file), "a+") as f:
            msg = "Epoch: {} | scenario: {} | AP @0.3: {:.04f} | AP @0.5: {:.04f} | AP @0.7: {:.04f} | comm_rate: {:.06f} \n".format(
                epoch_id, scenario, ap_30, ap_50, ap_70, comm_rates
            )
            if opt.comm_thre is not None:
                msg = "Epoch: {} | scenario: {} |  AP @0.3: {:.04f} | AP @0.5: {:.04f} | AP @0.7: {:.04f} | comm_rate: {:.06f} | comm_thre: {:.04f}\n".format(
                    epoch_id, scenario, ap_30, ap_50, ap_70, comm_rates, opt.comm_thre
                )
            f.write(msg)
            print(msg)
