...
def test_parser():
    ...
    parser.add_argument(
        "--save_pred",
        action="store_true",
        help="whether to save prediction result in pkl file as skylink format",
    )
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == "late":
                ...
                pred_box_tensor, pred_score, pred_labels, gt_box_tensor, gt_class_label_list, gt_track_list = (
                    inference_utils.inference_early_fusion_skylink(
                        batch_data, model, opencood_dataset
                    )
                )
            elif opt.fusion_method == "intermediate":
                pred_box_tensor, pred_score, pred_labels, gt_box_tensor, gt_class_label_list, gt_track_list = (
                    inference_utils.inference_intermediate_fusion_skylink(
                        batch_data, model, opencood_dataset
                    )
                )
            ...
            if opt.save_pred:
                ...
                inference_utils.save_preds_skylink(pred_box_tensor, pred_score, pred_labels, batch_data, saved_path)
