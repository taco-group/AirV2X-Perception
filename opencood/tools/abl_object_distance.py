...
def main():
    ...
    for idx, batch_data in enumerate(data_loader):
        ...
        for cav_id, cav_content in base_data_dict.items():
            visibility_map = np.asarray(
                cv2.cvtColor(cav_content["bev_visibility.png"], cv2.COLOR_BGR2GRAY)
            )
            object_bbx_center, object_bbx_mask, object_ids = (
                data_loader.dataset.generate_object_center(
    ...
    with open("distance.txt", "a") as f:
        f.write(
            "cavnum: {} min distance per vehicle: {} min distance per sample: {} \n".format(
                opt.cavnum, min_distances_perveh, min_distances_persample
            )
        )
