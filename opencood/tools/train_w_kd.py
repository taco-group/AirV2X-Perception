...
def main():
    ...
    if opt.model_dir:
        ...
        scheduler = train_utils.setup_lr_schedular(
            hypes, optimizer, init_epoch=init_epoch
        )
    ...
    supervise_single_flag = (
        True
        if "supervise_single" in hypes["train_params"]
        and hypes["train_params"]["supervise_single"]
        else False
    )
    ...
    if "kd_flag" in hypes.keys():
        kd_flag = True
        teacher_model_name = hypes["kd_flag"][
            "teacher_model"
        ...
        if epoch % hypes["train_params"]["eval_freq"] == 0:
            ...
            if valid_ave_loss < lowest_val_loss:
                ...
                if lowest_val_epoch != -1 and os.path.exists(
                    os.path.join(
                        saved_path, "net_epoch_bestval_at%d.pth" % (lowest_val_epoch)
                    )
                ):
                    os.remove(
                        os.path.join(
