...
def test_parser():
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to the .pth model file"
    )
    parser.add_argument(
        "--hypes_yaml",
        type=str,
    ...
    parser.add_argument(
        "--eval_epoch", type=int, default=None, help="Set the checkpoint"
    )
    parser.add_argument(
...
def main():
    ...
    ap_30, ap_50, ap_70 = eval_utils.eval_final_results(
        result_stat, os.path.dirname(opt.model_path)
    )
    ...
    yaml_save_path = os.path.join(
        os.path.dirname(opt.model_path), f"eval_{model_name}.yaml"
    )
