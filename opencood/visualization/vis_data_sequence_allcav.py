...
def gen_gt_bbox(cav_content):
    ...
    object_bbx_corner = box_utils.boxes_to_corners_3d(object_bbx_center, order)
    ...
    mask = box_utils.get_mask_for_boxes_within_range_torch(gt_box3d_tensor, gt_range)
...
if __name__ == "__main__":
    ...
    params = load_yaml(
        os.path.join(current_path, "../hypes_yaml/visualization_opv2v.yaml")
    )
    ...
    data = torch.load(
        "/GPFS/rhome/yifanlu/workspace/OpenCOOD/box_align_items/box_align.pt"
    )  ### Modified Here
    pred_corner3d_list, pred_box3d_list, scores_list, record_len, lidar_poses = (
        data  ### Modified Here
    )
