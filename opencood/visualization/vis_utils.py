...
def visualize_single_sample_output_gt(
    pred_tensor, gt_tensor, pcd, show_vis=True, save_path="", mode="constant"
):
    """
    ...
    origin_lidar_intcolor = color_encoding(
        origin_lidar[:, -1] if mode == "intensity" else origin_lidar[:, 2], mode=mode
    )
    # left -> right hand
...
def visualize_single_sample_output_bev(
    pred_box, gt_box, pcd, dataset, show_vis=True, save_path=""
):
    """
...
def visualize_single_sample_dataloader(
    ...
    aabbs = (
        bbx2linset(object_bbx_center, order)
        if not oabb
        else bbx2oabb(object_bbx_center, order)
    )
...
def visualize_inference_sample_dataloader(
    pred_box_tensor, gt_box_tensor, origin_lidar, o3d_pcd, mode="constant"
):
    """
    ...
    while True:
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            pcd, aabbs = visualize_single_sample_dataloader(
                sample_batched["ego"], vis_pcd, order, mode=color_mode
            )
            if i_batch == 0:
                vis.add_geometry(pcd)
...
def visualize_bev(batch_data):
    ...
    if not isinstance(label_map, np.ndarray):
        label_map = (
            label_map[0].numpy()
            if not label_map[0].is_cuda
            else label_map[0].cpu().detach().numpy()
        )
...
def draw_box_plt(boxes_dec, ax, color=None, linewidth_scale=1.0):
    ...
    new_x = (
        (corners[:, :, 0] - x[:, None]) * np.cos(theta)
        + (corners[:, :, 1] - y[:, None]) * (-np.sin(theta))
        + x[:, None]
    )
    new_y = (
        (corners[:, :, 0] - x[:, None]) * np.sin(theta)
        + (corners[:, :, 1] - y[:, None]) * (np.cos(theta))
        + y[:, None]
    )
