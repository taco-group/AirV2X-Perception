...
from opencood.models.cobevt_modules.base_transformer import FeedForward, PreNormResidual
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        out = self.to_out(out)
        return rearrange(
            out, "(b x y) l w1 w2 d -> b l x y w1 w2 d", b=batch, x=height, y=width
        )
...
class SwapFusionBlockMask(nn.Module):
    ...
    def __init__(self, input_dim, mlp_dim, dim_head, window_size, agent_size, drop_out):
        ...
        self.window_attention = PreNormResidual(
            input_dim, Attention(input_dim, dim_head, drop_out, agent_size, window_size)
        )
...
class SwapFusionBlock(nn.Module):
    ...
    def __init__(self, input_dim, mlp_dim, dim_head, window_size, agent_size, drop_out):
        ...
        self.block = nn.Sequential(
            Rearrange(
                "b m d (x w1) (y w2) -> b m x y w1 w2 d", w1=window_size, w2=window_size
            ),
            PreNormResidual(
                input_dim,
                Attention(input_dim, dim_head, drop_out, agent_size, window_size),
            ),
            PreNormResidual(input_dim, FeedForward(input_dim, mlp_dim, drop_out)),
            Rearrange("b m x y w1 w2 d -> b m d (x w1) (y w2)"),
            Rearrange(
                "b m d (w1 x) (w2 y) -> b m x y w1 w2 d", w1=window_size, w2=window_size
            ),
            PreNormResidual(
                input_dim,
                Attention(input_dim, dim_head, drop_out, agent_size, window_size),
