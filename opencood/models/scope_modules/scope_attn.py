...
from opencood.models.scope_modules.scope_deformable_transformer import (
        ...
        if self.multi_scale:
            ...
            for idx in range(self.num_levels):
                if self.agg_mode == "ATTEN":
                    ...
                    fuse_network = RPN_transformer_deformable_mtf_singlescale(
                        channel=num_filters[idx]
                    )
            ...
            for i in range(self.num_levels):
                ...
                if i == 0:
                    if self.communication:
                        batch_confidence_maps = self.regroup(rm, record_len)
                        _, communication_masks, communication_rates, deform_map = (
                ...
                for b in range(B):
                    ...
                    if self.agg_mode == "Deform":
                        ...
                        if i == 0:
                            ...
                            deform_map_b = torch.cat(
                                [deform_map[b], padding_map], dim=0
                            )
                        x_fuse.append(
                            self.fuse_modules[i](neighbor_feature, deform_map_b, i)
                        )
                    else:
...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, record_len, pairwise_t_matrix):
        ...
        deform_communication_map = []
        for b in range(B):
            N = record_len[b]
            ori_communication_maps = (
                batch_confidence_maps[b].sigmoid().max(dim=1)[0].unsqueeze(1)
            )  # dim1=2 represents the confidence of two anchors

            if self.smooth:
                communication_maps = self.gaussian_filter(ori_communication_maps)
                deform_communication_map.append(communication_maps)
            else:
