...
def vis_feature_denoise(lidar_batch_dict):
    channel_image = np.sum(
        lidar_batch_dict["de_spatial_features"].detach().cpu().numpy(), axis=(0, 1)
    )[:, 352:552]
    ...
    channel_image = np.sum(
        lidar_batch_dict["spatial_features"].detach().cpu().numpy(), axis=(0, 1)
    )[:, 352:552]
    channel_image /= channel_image.max()
    channel_image *= 255
    channel_image = np.clip(channel_image, 0, 255).astype("uint8")
    ...
    channel_image = np.sum(
        lidar_batch_dict["bf_spatial_features"].detach().cpu().numpy(), axis=(0, 1)
    )[:, 352:552]
    channel_image /= channel_image.max()
    channel_image *= 255
    channel_image = np.clip(channel_image, 0, 255).astype("uint8")
    ...
    channel_image = np.sum(
        lidar_batch_dict["ra_spatial_features"].detach().cpu().numpy(), axis=(0, 1)
    )[:, 352:552]
    channel_image /= channel_image.max()
    channel_image *= 255
    channel_image = np.clip(channel_image, 0, 255).astype("uint8")
...
class PointPillarIntermediateLRFMDD(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if data_dict["train"]:
            ...
            de_lidar_voxel_num_points = data_dict["processed_de_lidar"][
                "voxel_num_points"
            ]
            record_len = data_dict["record_len"]

            de_lidar_batch_dict = {
                "voxel_features": de_lidar_voxel_features,
            ...
            lidar_batch_dict["de_spatial_features"] = de_lidar_batch_dict[
                "spatial_features"
            ]
        ...
        lidar_batch_dict["spatial_features"] = lidar_batch_dict["pred_feature"] * (
            lidar_batch_dict["spatial_features"] != 0
        )
