...
from opencood.models.where2comm_modules.where2comm_attn import (
    ...
    def forward(self, x, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            regroup_feature_new.append(
                warp_affine_simple(
                    regroup_feature[b], pairwise_t_matrix[b, ego], (H, W)
                )
            )
...
class When2commFusion(nn.Module):
    def __init__(self, args):
        ...
        from opencood.models.fuse_modules.when2com_fuse import (
            AdditiveAttentin,
            MIMOGeneralDotProductAttention,
            km_generator_v2,
            policy_net4,
        )
    ...
    def forward(self, x, record_len, pairwise_t_matrix):
        ...
        for b in range(B):
            ...
            query = self.query_net(query_key_maps[0].unsqueeze(0)).unsqueeze(
                0
            )  # [1, C_q]
            ...
            feat_fuse, prob_action = self.attention_net(
                query, keys, neighbor_feature, sparse=False
            )
            ...
            for i in range(self.num_levels):
                ...
                if i == 0:
                    if self.communication:
                        ...
                        _, communication_masks, communication_rates = (
