...
def transform_feature(feature_list, matrix_list, downsample_rate, discrete_ratio):
    ...
    for b in range(B):
        ...
        history_matrix = torch.cat(
            [new_list[0][b, 0, 0:1, :, :], history_matrix], dim=0
        )
...
class PointPillarScope(nn.Module):
    def __init__(self, args):
        ...
        self.downsample_rate = args["fusion_args"][
            "downsample_rate"
    ...
    def forward(self, data_dict_list):
        ...
        if self.frame > 0:
            history_feature = transform_feature(
                regroup_feature_list_large,
                matrix_list,
                self.downsample_rate,
                self.discrete_ratio,
            )
            history_feature_2d = transform_feature(
                regroup_feature_list,
                matrix_list,
                self.downsample_rate,
                self.discrete_ratio,
        ...
        final_feature = self.late_fusion(
            [temporal_output, ego_feature, fused_feature],
            psm_temporal,
            psm_single_v,
            psm_cross,
        )
