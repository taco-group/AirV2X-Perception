...
class LiftSplatEncoder(nn.Module):
    def __init__(self, args):
        super(LiftSplatEncoder, self).__init__()
        self.grid_conf = args["image_modality"]["grid_conf"]  # 网格配置参数
        self.data_aug_conf = args["image_modality"]["data_aug_conf"]  # 数据增强配置参数
        self.bevout_feature = args["image_modality"]["bevout_feature"]
        dx, bx, nx = gen_dx_bx(
        ...
        self.downsample = args["image_modality"]["img_downsample"]
        self.camC = args["image_modality"]["img_features"]
        self.frustum = (
            self.create_frustum()
            .clone()
            .detach()
        ...
        self.camera_encoder_type = args["image_modality"]["camera_encoder"]
        if self.camera_encoder_type == "EfficientNet":
            self.camencode = CamEncode(
                ...
                self.grid_conf["mode"],
                args["image_modality"]["use_depth_gt"],
                args["image_modality"]["depth_supervision"],
            ).to("cuda")
        ...
        self.cls_head = nn.Conv2d(
            self.bevout_feature, args["num_anchor"], kernel_size=1
        )
        self.reg_head = nn.Conv2d(
            self.bevout_feature, 7 * args["num_anchor"], kernel_size=1
        )
    ...
    def forward(self, data_dict):
        ...
        # psm = self.cls_head(x)
        # rm = self.reg_head(x)
        # output_dict = {"cls_preds": psm, "reg_preds": rm, "depth_items": depth_items}
...
if __name__ == "__main__":
    ...
    B, N, C, H, W = (
        ...
        300,
        400,
    )  # Batch size, Number of cameras, Channels, Height, Width

    # Generate dummy inputs
    image_inputs_dict = {
        "image_inputs": {
            "imgs": torch.randn(B, N, C, H, W).to("cuda"),  # Image tensors
            "rots": torch.eye(3)
            .expand(B, N, 3, 3)
            .clone()
            .to("cuda"),  # Identity rotation matrices
            "trans": torch.zeros(B, N, 3).to("cuda"),  # Zero translation vectors
            "intrins": torch.eye(3)
            .expand(B, N, 3, 3)
            .clone()
            .to("cuda"),  # Identity intrinsics
            "post_rots": torch.eye(3)
            .expand(B, N, 3, 3)
            .clone()
            .to("cuda"),  # Identity post-rotation
            "post_trans": torch.zeros(B, N, 3).to("cuda"),  # Zero post-translation
        }
    }

    model = LiftSplatEncoder(config)
    out = model(image_inputs_dict)
