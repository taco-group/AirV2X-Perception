...
from opencood.models.v2vnet_modules.convgru import ConvGRU
...
class V2VNetFusion(nn.Module):
    ...
    def forward(self, x, record_len, pairwise_t_matrix, weight=None):
        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                for i in range(N):
                    ...
                    comm_rates.append(batch_node_features[b].count_nonzero().item())
                    # print(batch_node_features[b][0].count_nonzero().item(), batch_node_features[b].shape, comm_rates[-1])
                    ...
                    if self.agg_operator == "avg":
                        ...
                        agg_feature = torch.sum(
                            message * weight[b][i, :N].view(-1, 1, 1, 1), dim=0
                        )
                    else:
        ...
        comm_rates = np.sum(comm_rates) / B
        # (B,C,H,W)
        out = torch.cat(
