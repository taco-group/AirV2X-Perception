...
class PointPillarDiscoNet(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        pairwise_t_matrix[..., 0, 2] = (
            pairwise_t_matrix[..., 0, 2] / (W0 * self.discrete_ratio) * 2
        )
        pairwise_t_matrix[..., 1, 2] = (
            pairwise_t_matrix[..., 1, 2] / (H0 * self.discrete_ratio) * 2
        )
        ...
        output_dict = {"feature": spatial_features_2d, "psm": psm, "rm": rm}
        output_dict.update({"mask": 0, "comm_rate": np.sum(comm_rates) / B})
