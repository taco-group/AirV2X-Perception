...
class CamEncode(nn.Module):  # 提取图像特征进行图像编码
    def __init__(
        ...
        self.chain_channels = 256  # TODO(YH): set for now
    ...
    def get_gt_depth_dist(self, x):  # 对深度维进行onehot，得到每个像素不同深度的概率
        ...
        onehot_dist = F.one_hot(depth_indices.long(),
    ...
    def get_eff_features(self, x):  # 使用efficientnet提取特征
        ...
        )  #  x: 24 x 32 x 64 x 176 (BN, C', H/2, W/2)
        prev_x = x
    ...
    def forward(self, x):
        ...
        x_img_ = x[
            :, :3:, :, :
        ]  # [B*N, 4, H, W], the first 3 are RGB, and 4th is depth map(either gt or to be predicted)
        ...
        if self.use_gt_depth:
            new_x = depth_gt.unsqueeze(1) * x_img.unsqueeze(
                2
            )  # new_x: 24 x 64 x 41 x 8 x 18
            return None, new_x
        else:
            depth_logit = self.depth_head(features)
            depth = self.get_depth_dist(depth_logit)
            new_x = depth.unsqueeze(1) * x_img.unsqueeze(
                2
            )  # new_x: 24 x 64 x 41 x 8 x 18
...
class CamEncode_Resnet101(nn.Module):  # 提取图像特征进行图像编码
    def __init__(
        ...
        if not use_gt_depth:
            self.depth_head = nn.Conv2d(
                512, self.D, kernel_size=1, padding=0
            )  # 1x1卷积，变换维度
    ...
    def get_gt_depth_dist(self, x):  # 对深度维进行onehot，得到每个像素不同深度的概率
        ...
        onehot_dist = F.one_hot(depth_indices.long()).permute(
            0, 3, 1, 2
        )  # [B*N, num_bins, fH, fW]
    ...
    def forward(self, x):
        ...
        if self.use_gt_depth:
            new_x = depth_gt.unsqueeze(1) * x_img_feature.unsqueeze(
                2
            )  # new_x: 24 x 64 x 41 x 8 x 18
            return None, new_x
        else:
            depth_logit = self.depth_head(features)
            depth = self.get_depth_dist(depth_logit)
            new_x = depth.unsqueeze(1) * x_img_feature.unsqueeze(
                2
            )  # new_x: 24 x 64 x 41 x 8 x 18
            if self.depth_supervision:
                return (depth_logit, depth_gt_indices), new_x
...
class BevEncode(nn.Module):
    ...
    def forward(self, x):  # x: 4 x 64 x 240 x 240
        ...
        x = self.up1(
            x, x1
        )  # 给x进行4倍上采样然后和x1 concat 在一起  x: 4 x 256 x 120 x 120
...
class BevEncodeSSFusion(nn.Module):
    ...
    def forward(self, x, record_len, pairwise_t_matrix):  # x: 4 x 64 x 240 x 240
        ...
        pairwise_t_matrix = normalize_pairwise_tfm(
            pairwise_t_matrix, H, W, self.discrete_ratio, self.downsample_rate
        )
        ...
        x_single = self.down_layer(
            self.up_layer1(self.up_layer2(x3, x2), x1)
        )  # 4 x 128 x 120 x 120
...
class BevEncodeMSFusion(nn.Module):
    ...
    def forward(self, x, record_len, pairwise_t_matrix):  # x: 4 x 64 x 240 x 240
        ...
        x_fuse = self.down_layer(
            self.up_layer1(self.up_layer2(x3_fuse, x2_fuse), x1_fuse)
        )  # 4 x 64 x 120 x 120
