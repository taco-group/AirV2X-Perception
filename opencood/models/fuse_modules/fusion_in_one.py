...
class V2VNetFusion(nn.Module):
    def __init__(self, args):
        ...
        H, W = (
            args["conv_gru"]["H"],
            args["conv_gru"]["W"],
        )  # remember to modify for v2xsim dataset
        ...
        for b in range(B):
            ...
            for i in range(N):
                ...
                roi_mask[b, i] = warp_affine_simple(
                    one_tensor, affine_matrix[b][i, :, :, :], (H, W)
                )
        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                for i in range(N):
                    ...
                    # (N,C,H,W)
                    ego_agent_feature = (
                        batch_node_features[b][i].unsqueeze(0).repeat(N, 1, 1, 1)
                    )
                    # (N,2C,H,W)
                    ...
                    if self.gru_flag:
                        gru_out = (
                            self.conv_gru(cat_feature.unsqueeze(0).unsqueeze(0))[0][0]
                            .squeeze(0)
                            .squeeze(0)
                        )
                    else:
    ...
    def __init__(self, args):
        ...
        for i in range(self.depth):
            block = SwapFusionBlockMask(
                input_dim, mlp_dim, dim_head, window_size, agent_size, drop_out
            )
...
class Where2commFusion(nn.Module):
    ...
    def forward(self, x, record_len, affine_matrix):
        ...
        for b in range(B):
            ...
            # to [1, H*W, C]
            fused_feature = self.mha_fusion(
                neighbor_feature[0:1].permute(0, 2, 3, 1).flatten(1, 2),
...
class Who2comFusion(nn.Module):
    def __init__(self, feature_dims):
        ...
        self.att = ScaledDotProductAttention(feature_dims)
        self.decode_layer = nn.Conv2d(
            feature_dims * 2, feature_dims, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, x, record_len, affine_matrix):
        ...
        for b in range(B):
            ...
            neighbor_feature = self.att(
                neighbor_feature, neighbor_feature, neighbor_feature
            )
            neighbor_feature = neighbor_feature.permute(1, 2, 0).view(cav_num, C, H, W)[
                0, ...
            ]  # [C, H, W]

            concat_feature = torch.cat(
                (ego_feature, neighbor_feature), dim=0
            ).unsqueeze(0)  # [1, 2C, H, W]
