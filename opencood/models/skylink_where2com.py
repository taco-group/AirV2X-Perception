...
class SkylinkWhere2com(SkylinkBase):
    ...
    def forward(self, data_dict):
        ...
        if self.args["task"] == "det":
            ...
            # cv2.imwrite("/home/xiangbog/Folder/Research/skylink/debug/debug_image_dynamic.png", ((dynamic - dynamic.min()) / (dynamic.max() - dynamic.min()) * 255).astype(np.uint8),)
            ...
            # cv2.imwrite("/home/xiangbog/Folder/Research/skylink/debug/debug_image_segfeat.png", ((feat - feat.min()) / (feat.max() - feat.min()) * 255).astype(np.uint8),)
            
            # feat = batch_spatial_features_2d[0].mean(0).detach().cpu().numpy()
            # import cv2; import numpy as np
            # cv2.imwrite("/home/xiangbog/Folder/Research/skylink/debug/debug_image_bevfeat.png", ((feat - feat.min()) / (feat.max() - feat.min()) * 255).astype(np.uint8),)
