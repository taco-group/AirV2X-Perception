...
def get_roi_and_cav_mask(
    shape, cav_mask, spatial_correction_matrix, discrete_ratio, downsample_rate
):
    """
...
def get_rotated_roi(shape, correction_matrix):
    ...
    # (B,L,1,H,W)
    x = (
        torch.ones((B, L, 1, H, W))
        .to(correction_matrix.dtype)
        .to(correction_matrix.device)
    )
    # (B*L,1,H,W)
    roi_mask = warp_affine(
        x.reshape(-1, 1, H, W), correction_matrix, dsize=(H, W), mode="nearest"
    )
    # (B,L,C,H,W)
...
def _torch_inverse_cast(input):
    ...
    if dtype not in (torch.float32, torch.float64):
        dtype = torch.float32
    if input.device != "cpu":
        inv_input = torch.inverse(input.to("cpu")).to(input.device)
    else:
        inv_input = torch.inverse(input)
    out = inv_input.to(input.dtype)
    return out
...
def normalize_homography(dst_pix_trans_src_pix, dsize_src, dsize_dst=None):
    ...
    src_norm_trans_src_pix = normal_transform_pixel(src_h, src_w, device, dtype).to(
        dst_pix_trans_src_pix
    )

    src_pix_trans_src_norm = _torch_inverse_cast(src_norm_trans_src_pix)
    dst_norm_trans_dst_pix = normal_transform_pixel(dst_h, dst_w, device, dtype).to(
        dst_pix_trans_src_pix
    )
...
def warp_affine_simple(
    src, M, dsize, mode="bilinear", padding_mode="zeros", align_corners=False
):
    B, C, H, W = src.size()
    grid = F.affine_grid(M, [B, C, dsize[0], dsize[1]], align_corners=align_corners).to(
        src
    )
...
def warp_affine(
    src, M, dsize, mode="bilinear", padding_mode="zeros", align_corners=True
):
    r"""
...
class Test:
    ...
    def test_combine_roi_and_cav_mask():
        ...
        mask = get_roi_and_cav_mask(
            (B, L, H, W, C), cav_mask, correction_matrix, 0.4, 4
        )
