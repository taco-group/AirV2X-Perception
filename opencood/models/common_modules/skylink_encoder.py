...
class LiftSplatShootEncoder(nn.Module):
    ...
    def get_voxels(self, x, rots, trans, intrins, post_rots, post_trans, i=None):
        ...
        if VISUALIZE:
            ...
            depth = bin_depths(depth, self.grid_conf["mode"], self.grid_conf["ddiscr"][0], self.grid_conf["ddiscr"][1], self.grid_conf["ddiscr"][2], target=True)[0]
            cv2.imwrite(f"/home/xiangbog/Folder/Research/SkyLink/skylink/debug/depth_encoded_{i}.png", depth[0,0].squeeze().cpu().numpy().astype(np.uint8))
            depth = depth_to_one_hot(depth)
            # import pdb; pdb.set_trace()
            visualize_3d_points(geom, depth, f"/home/xiangbog/Folder/Research/SkyLink/skylink/debug/point_cloud_v1_{i}.png")
            

        x = self.voxel_pooling(geom, x_img)  # x: 4 x 64 x 240 x 240
        # depth = self.voxel_pooling(geom, depth)  # depth: 4 x 1 x 240 x 240

        # return x, depth_items, depth
    ...
    #     )
    #     # x, depth_items, depth = self.get_voxels(
    #     #     x, rots, trans, intrins, post_rots, post_trans
    #     # )  # 将图像转换到BEV下，x: B x C x 240 x 240 (4 x 64 x 240 x 240)
    #     xs = []
    #     for i in range(x.shape[1]):
    #         x_i, depth_items_i, depth_i = self.get_voxels(
    #             x[:,i].unsqueeze(1),
    #             rots[:,i].unsqueeze(1),
    #             trans[:,i].unsqueeze(1),
    #             intrins[:,i].unsqueeze(1),
    #             post_rots[:, i].unsqueeze(1),
    #             post_trans[:, i].unsqueeze(1),
    #             i
    #         )
    #         if VISUALIZE:
    #             sp = "/home/xiangbog/Folder/Research/SkyLink/skylink/debug/"
    #             cv2.imwrite(sp+f'depth_{i}.png', image_inputs_dict["imgs"][0, i, 3].cpu().numpy().astype(np.uint8))
    #             visualize_bev_feature(x_i!=0, batch_idx=0, channel_idx=None, save_path=sp+f'bev_feature_{i}.png')
    #         xs.append(x_i)  # x_i: 4 x 64 x 240 x 240
            
    #     if VISUALIZE:
    #         visualize_bev_feature_in_once(sp)
            
    #     x = torch.cat(xs, dim=0)  # x: 4 x 64 x 240 x 240
