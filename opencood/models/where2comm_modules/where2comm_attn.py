    ...
    def forward(self, q, k, v, confidence_map=None):
        ...
        if confidence_map is not None:
            context, weight = self.attn(
                q, k, v, quality_map=confidence_map
            )  # (1, H*W, C)
        else:
...
class TransformerFusion(nn.Module):
    ...
    def forward(
        self,
        batch_neighbor_feature,
        batch_neighbor_feature_pe,
        batch_confidence_map,
        record_len,
    ):
        x_fuse = []
        B = len(record_len)
        for b in range(B):
            ...
            if self.with_spe:
                ...
                neighbor_feature_flat_pe = neighbor_feature_pe.view(
                    N, C, H * W
                )  # (N, C, H*W)
                query = neighbor_feature_flat_pe[0:1, ...].permute(
                    0, 2, 1
                )  # (1, H*W, C)
            ...
            if self.with_scm:
                ...
                fused_feature = self.encode_layer(
                    query, key, value, confidence_map
                )  # (1, H*W, C)
            else:
...
def add_pe_map(x):
    ...
    dim_t = torch.arange(
        num_pos_feats, dtype=torch.float32, device=x.device
    )  # [0,1,2,...,d]
    dim_t = temperature ** (
        2 * (dim_t // 2) / num_pos_feats
    )  # 10000^(2k/d), k is [0,0,1,1,...,d/2,d/2]
        ...
        if self.multi_scale:
            ...
            if self.communication:
                _, communication_masks, communication_rates = self.naive_communication(
                    batch_x, batch_confidence_maps, record_len, pairwise_t_matrix
                )
            else:
                communication_rates = torch.tensor(0).to(x.device)
