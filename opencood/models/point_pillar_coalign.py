...
from opencood.models.coalign_modules.fusion_in_one import (
    Att_w_Warp,
...
class PointPillarCoAlign(nn.Module):
    def __init__(self, args):
        ...
        self.backbone = ResNetBEVBackbone(args["res_bev_backbone"], 64)
        ...
        for i in range(len(args["res_bev_backbone"]["layer_nums"])):
            ...
            self.fusion_net.append(
                Att_w_Warp(args["res_bev_backbone"]["num_filters"][i])
            )
        ...
        if "compression" in args:
            ...
            self.naive_compressor = NaiveCompressor(
                args["res_bev_backbone"]["num_filters"][0], args["compression"]
            )
        ...
        self.reg_head = nn.Conv2d(
            self.out_channel, 7 * args["anchor_num"], kernel_size=1
        )
