...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, kv):
        ...
        kv = (
            self.kv(kv)
            .reshape(B, T, 2, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )  # (2, B, heads, T, C//heads)
...
class Denosier(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.crossblocks = nn.ModuleList(
            [CrossBlock(embed_dim, num_heads=4) for _ in range(self.depth)]
        )
    ...
    def forward(self, feat, noisy_masks, upsam=False, t=None):
        ...
        resize_scale = 0.25
        cur_feat = F.interpolate(
            feat, scale_factor=resize_scale, mode="bilinear", align_corners=False
        )
...
class Cond_Diff_Denoise(nn.Module):
    ...
    def p_mean_variance(self, feat, upsam, noisy_masks, t, clip_denoised: bool):
        ...
        if upsam:  # last sampling step
            ...
            x_recon = F.interpolate(
                x_recon, x.shape[2:], mode=INTERPOLATE_MODE, align_corners=False
            )
    ...
    def p_sample(
        self, feat, noisy_masks, t, upsam, clip_denoised=False, repeat_noise=False
    ):
        model_mean, _, model_log_variance, model_out = self.p_mean_variance(
            feat, upsam, noisy_masks, t=t, clip_denoised=clip_denoised
        )
    ...
    def p_sample_loop(self, feat, noisy_masks, latent_shape):
        ...
        for t in reversed(range(0, num_timesteps)):
            noisy_masks = F.interpolate(
                noisy_masks,
                latent_shape[2:],
                mode=INTERPOLATE_MODE,
                align_corners=False,
            )
            noisy_masks = self.p_sample(
                feat,
                noisy_masks,
        ...
        if self.training:
            ...
            for b_idx in range(bs):
                ...
                for _t in reversed(range(1, self.num_timesteps)):
                    _t = torch.full(
                        (x.shape[0],), _t, device=x.device, dtype=torch.long
                    )
