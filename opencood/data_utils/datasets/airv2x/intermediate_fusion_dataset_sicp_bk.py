...
class IntermediateFusionDatasetAirv2xSiCP(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            if (
                agent_type in ["rsu", "vehicle"]
                and distance > opencood.data_utils.datasets.VEH_RSU_COM_RANGE
            ):
                continue
            elif (
                agent_type == "drone"
        ...
        pairwise_t_matrix_collab, img_pairwise_t_matrix_collab = (
            self.get_pairwise_transformation(
                base_data_dict, self.max_cav_num, self.collaborators
            )
        )
        ...
        processed_data_dict["ego"].update(
            {
                ...
                # TODO(YH): seg should be merged if collab
                "seg_label": seg_label,
                "seg_label_ego": seg_label,
    ...
    def collate_batch_train(self, batch):
        ...
        seg_label_torch = torch.from_numpy(np.array(seg_label_list))
        ...
        ego_label_dict_torch.update({"seg_label": seg_label_torch})
