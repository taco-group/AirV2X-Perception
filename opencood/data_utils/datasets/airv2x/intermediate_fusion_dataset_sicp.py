...
class IntermediateFusionDatasetAirv2xSiCP(basedataset.BaseDataset):
    """
    ...
    def __init__(self, params, visualize, train=True):
        super(IntermediateFusionDatasetAirv2xSiCP, self).__init__(
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            if selected_cav_base["ego"]:
                ...
                ego_object_stack.append(selected_cav_processed["object_bbx_center"])
                ego_object_id_stack.append(selected_cav_processed["object_ids"])
                ego_class_id_stack.append(selected_cav_processed["class_ids"])
        ...
        # veh
        veh_indices, retain_veh_ids = airv2x_utils.get_smallest_k_indices(
        ...
        # rsu
        rsu_indices, retain_rsu_ids = airv2x_utils.get_smallest_k_indices(
        ...
        # drone
        drone_indices, retain_drone_ids = airv2x_utils.get_smallest_k_indices(
        ...
        # =============================================================
        # SiCP needs ego label
        # =============================================================
        ego_object_stack = np.vstack(list(chain.from_iterable(ego_object_stack)))
        ego_class_id_stack = np.array(list(chain.from_iterable(ego_class_id_stack)))

        ego_object_bbx_center = np.zeros((self.params["postprocess"]["max_num"], 7))
        ego_object_mask = np.zeros(self.params["postprocess"]["max_num"])
        ego_object_bbx_center[: ego_object_stack.shape[0], :] = ego_object_stack
        ego_object_mask[: ego_object_stack.shape[0]] = 1
        ego_class_ids_padded = np.full(self.params["postprocess"]["max_num"], 0)
        ego_class_ids_padded[: ego_object_stack.shape[0]] = ego_class_id_stack
        ego_anchor_box = self.post_processor.generate_anchor_box()

        ego_label_dict = self.post_processor.generate_label_airv2x(
            gt_box_center=ego_object_bbx_center,
            anchors=ego_anchor_box,
            mask=ego_object_mask,
            class_ids_padded=ego_class_ids_padded,
        )
        ...
        processed_data_dict["ego"].update(
            {
                ...
                "dynamic_seg_label_ego": dynamic_seg_label,
                "static_seg_label_ego": static_seg_label, 
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        for idx, img in enumerate(camera_data_list):
            ...
            # depth_img = np.clip(np.array(depth_img_bk),0, 16776/4); cv2.imwrite("/home/xiangbog/Folder/Research/SkyLink/airv2x/debug/debug_image_v2.png", ((depth_img - depth_img.min()) / (depth_img.max() - depth_img.min()) * 255).astype(np.uint8),)
            ...
            # # img_src[1] = np.array(img_src[1]); img_src[1] = img_src[1][0]; cv2.imwrite("/home/xiangbog/Folder/Research/SkyLink/airv2x/debug/debug_image_v3.png", ((img_src[1] - img_src[1].min()) / (img_src[1].max() - img_src[1].min()) * 255).astype(np.uint8),)
    ...
    def collate_batch_train(self, batch):
        ...
        # used for SiCP
        ego_label_dict_list = []
        dynamic_seg_label_ego_list = []
        static_seg_label_ego_list = []
        ...
        for i in range(len(batch)):
            ...
            # SiCP
            ego_label_dict_list.append(ego_dict["ego_label_dict"])
            dynamic_seg_label_ego_list.append(ego_dict["dynamic_seg_label_ego"])
            static_seg_label_ego_list.append(ego_dict["static_seg_label_ego"])
        ...
        dynamic_seg_label_ego_torch = torch.from_numpy(np.array(dynamic_seg_label_ego_list))
        static_seg_label_ego_torch = torch.from_numpy(np.array(static_seg_label_ego_list))
        ...
        ego_label_dict_torch = self.post_processor.collate_batch_airv2x(
            ego_label_dict_list
        )
        ego_label_dict_torch.update({"dynamic_seg_label": dynamic_seg_label_ego_torch, "static_seg_label": static_seg_label_ego_torch})
        ...
        output_dict["ego"].update(
            {
                ...
                "label_dict_ego": ego_label_dict_torch,
                "vehicle": {
...
if __name__ == "__main__":
    params = load_yaml(
        "/code/opencood/hypes_yaml/airv2x/airv2x_intermediate_bm2cp.yaml"
    )
    airv2x_database = IntermediateFusionDatasetAirv2xSiCP(params, None)
