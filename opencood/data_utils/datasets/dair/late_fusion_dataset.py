...
class LateFusionDatasetDAIR(Dataset):
    ...
    def retrieve_base_data(self, idx):
        ...
        data[0]["params"]["vehicles"] = load_json(
            os.path.join(
                self.root_dir, "vehicle-side/label/lidar/", veh_frame_id + ".json"
            )
        )
        ...
        data[1]["params"]["vehicles"] = load_json(
            os.path.join(
                self.root_dir,
        ...
        transformation_matrix1 = inf_side_rot_and_trans_to_trasnformation_matrix(
    ...
    def generate_object_center(self, cav_contents, reference_lidar_pose):
        ...
        return self.post_processor.generate_object_center_dairv2x_late_fusion(
            cav_contents[0]
        )
    ...
    def collate_batch_test(self, batch):
        ...
        if self.visualize:
            ...
            output_dict["ego"].update(
                {"origin_lidar_v": [torch.from_numpy(projected_lidar_list[0])]}
            )
            output_dict["ego"].update(
                {"origin_lidar_i": [torch.from_numpy(projected_lidar_list[1])]}
            )
