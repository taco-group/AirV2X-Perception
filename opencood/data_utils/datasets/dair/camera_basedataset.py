...
class CameraBaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.supervise_single = (
            True
            if (
                "supervise_single" in params["train_params"]
                and params["train_params"]["supervise_single"]
            )
            else False
        )
        ...
        self.preload = (
            True
            if (
                "preload" in params["train_params"]
                and params["train_params"]["preload"]
            )
            else False
        )
        ...
        self.use_fg_mask = (
            True
            if (
                "use_fg_mask" in params["loss"]["args"]
                and params["loss"]["args"]["use_fg_mask"]
            )
            else False
        )
        ...
        self.add_data_extension = (
            params["add_data_extension"] if "add_data_extension" in params else []
        )
                ...
                for timestamp in timestamps:
                    ...
                    for file_extension in self.add_data_extension:
                        file_name = os.path.join(
                            cav_path, timestamp + "_" + file_extension
                        )

                        self.scenario_database[i][cav_id][timestamp][file_extension] = (
                            file_name
                        )
    ...
    def retrieve_base_data(self, idx):
        ...
        for cav_id in cav_id_list:
            ...
            if (
                self.select_keypoint
                and "lidar_keypoints_np" in cav_content[timestamp_key]
            ):
                data[cav_id]["lidar_keypoints_np"] = cav_content[timestamp_key][
                    "lidar_keypoints_np"
                ]

            hdf5_file = cav_content[timestamp_key]["cameras"][0].replace(
                "camera0.png", "imgs_hdf5"
            )
            if os.path.exists(hdf5_file):
                ...
                with h5py.File(hdf5_file, "r") as f:
                    ...
                    for i in range(4):
                        data[cav_id]["camera_data"].append(
                            Image.fromarray(f[f"camera{i}"][()])
                        )
                        data[cav_id]["depth_data"].append(
                            Image.fromarray(f[f"depth{i}"][()])
                        )

            else:
                data[cav_id]["camera_data"] = load_camera_data(
                    cav_content[timestamp_key]["cameras"], self.preload
                )
                if self.use_gt_depth:
                    data[cav_id]["depth_data"] = load_camera_data(
                        cav_content[timestamp_key]["depths"], self.preload
                    )  # we can use the same loading api
            ...
            for file_extension in self.add_data_extension:
                ...
                if not os.path.exists(cav_content[timestamp_key][file_extension]):
                    cav_content[timestamp_key][file_extension] = cav_content[
                        timestamp_key
                    ][file_extension].replace("train", "additional/train")
                    cav_content[timestamp_key][file_extension] = cav_content[
                        timestamp_key
                    ][file_extension].replace("validate", "additional/validate")
                    cav_content[timestamp_key][file_extension] = cav_content[
                        timestamp_key
                    ][file_extension].replace("test", "additional/test")

                if ".yaml" in file_extension:
                    data[cav_id][file_extension] = load_yaml(
                        cav_content[timestamp_key][file_extension]
                    )
                else:
                    data[cav_id][file_extension] = cv2.imread(
                        cav_content[timestamp_key][file_extension]
                    )
    ...
    def retrieve_all_base_data_mp(self):
        ...
        for i in range(self.preload_worker_num):
            p = multiprocessing.Process(
                target=self.retrieve_per_process, args=(i, idx_list_split[i], lock)
            )
    ...
    def load_depth_files(cav_path, timestamp):
        ...
        camera_files : list
            The list containing all camera png file paths.
        """
        depth0_file = os.path.join(cav_path, timestamp + "_depth0.png")
        depth1_file = os.path.join(cav_path, timestamp + "_depth1.png")
        depth2_file = os.path.join(cav_path, timestamp + "_depth2.png")
        depth3_file = os.path.join(cav_path, timestamp + "_depth3.png")
            ...
            for j, cav_id in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    yaml_file = yaml_file.replace(
                        "OPV2V_MoreAgents", "OPV2V_MoreAgents/dataset_annos"
                    )
