...
class IntermediateFusionDatasetV2(basedataset.BaseDataset):
    ...
    def collate_batch_train(self, batch):
        ...
        label_dict_no_coop_list_ = [
            label_dict
            for label_list in label_dict_no_coop_list
            for label_dict in label_list
        ]
        for i in range(len(label_dict_no_coop_list_)):
            if isinstance(label_dict_no_coop_list_[i], list):
                print("debug")
        label_no_coop_torch_dict = self.post_processor.collate_batch(
            label_dict_no_coop_list_
        )
        ...
        for b in range(len(batch)):
            for points in origin_lidar[b]:
                assert len(points) != 0
                coor_pad = np.pad(
                    points, ((0, 0), (1, 0)), mode="constant", constant_values=idx
                )
