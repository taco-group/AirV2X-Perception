...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            selected_cav_processed = self.get_item_single_car(
                selected_cav_base, ego_lidar_pose, ego_params
            )
        ...
        if self.fov:
            for boxes, object_id, points, r_points in zip(
                object_stack,
                object_id_stack,
                projected_lidar_stack,
                projected_radar_stack,
                ...
                if self.use_Lidar and self.use_Radar:
                    ...
                    point_indices = points_in_boxes_cpu(
                        points[:, :3], boxes[:, [0, 1, 2, 5, 4, 3, 6]]
                    )
                    r_point_indices = points_in_boxes_cpu(
                        r_points[:, :3], boxes[:, [0, 1, 2, 5, 4, 3, 6]]
                    )
        ...
        merged_lidar_feature_dict = self.merge_features_to_dict(
            processed_lidar_features
        )
        merged_radar_feature_dict = self.merge_features_to_dict(
            processed_radar_features
        )
        if self.use_DeLidar:
            merged_de_lidar_feature_dict = self.merge_features_to_dict(
                processed_de_lidar_features
            )
        ...
        processed_data_dict["ego"].update(
            {
                ...
                "processed_de_lidar": merged_de_lidar_feature_dict
                if self.use_DeLidar
                else None,
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, ego_params):
        ...
        radar_np[:, :3] = box_utils.project_points_by_matrix_torch(
            radar_np[:, :3], transformation_matrix
        )
        radar_np = mask_points_by_range(
            radar_np, self.params["preprocess"]["cav_lidar_range"]
        )
        if self.fov:
        ...
        if self.use_DeLidar:
            ...
            # remove points that hit itself
            de_lidar_np = mask_points_by_range(
                de_lidar_np, self.params["preprocess"]["cav_lidar_range"]
            )
            de_lidar_np = mask_ego_points(de_lidar_np)
            # project the lidar to ego space
            if self.proj_first:
                de_lidar_np[:, :3] = box_utils.project_points_by_matrix_torch(
                    de_lidar_np[:, :3], transformation_matrix
                )

            if self.fov:
                de_lidar_np = self.mask_ego_fov_flag(
                    selected_cav_base, de_lidar_np, ego_params
                )
                ...
                while (
                    processed_de_lidar["voxel_features"].shape[0] == 0
                    or processed_de_lidar["voxel_coords"].shape[0] == 0
                ):
                    de_lidar_np = np.array(
                        [
                            random.choices(range(0, 70), k=4),
                            random.choices(range(0, 70), k=4),
                            random.choices(range(0, 70), k=4),
                            random.choices(range(0, 70), k=4),
                            random.choices(range(0, 70), k=4),
        ...
        while (
            processed_lidar["voxel_features"].shape[0] == 0
            or processed_lidar["voxel_coords"].shape[0] == 0
        ):
            lidar_np = np.array(
                [
                    random.choices(range(0, 70), k=4),
            ...
            processed_lidar = self.pre_processor.preprocess(lidar_np)
        while (
            processed_radar["voxel_features"].shape[0] == 0
            or processed_radar["voxel_coords"].shape[0] == 0
        ):
            radar_np = np.array(
                [
                    random.choices(range(0, 70), k=4),
        ...
        selected_cav_processed.update(
            {
                ...
                "processed_de_lidar_features": processed_de_lidar
                if self.use_DeLidar
                else None,
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            projected_lidar.append(ego_dict["projected_lidar"])
            projected_radar.append(ego_dict["projected_radar"])
        ...
        if self.use_DeLidar:
            ...
            processed_de_lidar_torch_dict = self.pre_processor.collate_batch(
                merged_de_lidar_feature_dict
            )

        merged_radar_feature_dict = self.merge_features_to_dict(processed_radar_list)
        processed_radar_torch_dict = self.pre_processor.collate_batch(
            merged_radar_feature_dict
        )
        # [2, 3, 4, ..., M], M <= max_cav
        ...
        output_dict["ego"].update(
            {
                ...
                "processed_de_lidar": processed_de_lidar_torch_dict
                if self.use_DeLidar
                else None,
