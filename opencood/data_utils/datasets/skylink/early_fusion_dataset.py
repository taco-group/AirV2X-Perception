...
class EarlyFusionDatasetSkylink(basedataset.BaseDataset):
    """
    ...
    def __init__(self, params, visualize, train=True):
        super(EarlyFusionDatasetSkylink, self).__init__(params, visualize, train)
    ...
    def __getitem__(self, idx):
        base_data_dict, _, _ = self.retrieve_base_data(idx)
        ...
        for cav_id, cav_content in base_data_dict.items():
            if cav_content["ego"]:
                ego_id = cav_id
                # print(cav_content['params']["lidar"].keys())
                ego_lidar_pose = cav_content["params"]["lidar"]["lidar_pose"]
                break
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            class_id_stack += selected_cav_processed["class_ids"]
        ...
        # data augmentation
        projected_lidar_stack, object_bbx_center, mask = self.augment(
            projected_lidar_stack, object_bbx_center, mask
        )

        # we do lidar filtering in the stacked lidar
        projected_lidar_stack = mask_points_by_range(
            projected_lidar_stack, self.params["preprocess"]["cav_lidar_range"]
        )
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # print(selected_cav_base["params"]["lidar"]["lidar_pose"])
        transformation_matrix = x1_to_x2(
            selected_cav_base["params"]["lidar"]["lidar_pose"], ego_pose
        )
    ...
    def collate_batch_test(self, batch):
        ...
        for cav_id, cav_content in batch.items():
            ...
            # label dictionary
            label_torch_dict = self.post_processor.collate_batch_skylink(
