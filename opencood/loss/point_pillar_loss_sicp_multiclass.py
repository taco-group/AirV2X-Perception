...
class PointPillarLossSiCPMulticlass(nn.Module):
    def __init__(self, args):
        super(PointPillarLossSiCPMulticlass, self).__init__()
    ...
    def forward(self, output_dict, target_dict):
        ...
        rm = output_dict["rm"]
        psm = output_dict["psm"]
        obj = output_dict["obj"]  # [B, #anchor, 50, 176]
        ...
        total_loss = reg_loss + conf_loss + ego_flow_loss + obj_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        obj_loss = self.loss_dict["obj_loss"]
        if pbar is None:
            print(
                "[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f || Obj Loss: %.4f"
                % (
                    epoch,
                    batch_id + 1,
                    batch_len,
        ...
        writer.add_scalar("Objectness_loss", obj_loss, epoch * batch_len + batch_id)
