...
class VanillaSegLoss(nn.Module):
    def __init__(self, args):
        ...
        if self.seg_branch == 'dynamic':
            ...
            raise ValueError('The seg branch should be dynamic, static or both')
    ...
    def forward(self, output_dict, gt_dict):
        ...
        if self.seg_branch == 'dynamic':
            dynamic_gt = gt_dict['dynamic_seg_label'].long()
            dynamic_loss = self.loss_func_dynamic(dynamic_pred, dynamic_gt)
        elif self.seg_branch == 'static':
            static_gt = gt_dict['static_seg_label'].long()
