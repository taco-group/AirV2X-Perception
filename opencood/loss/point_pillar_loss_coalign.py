...
class PointPillarLossCoAlign(nn.Module):
    def __init__(self, args):
        super(PointPillarLossCoAlign, self).__init__()
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # cls loss
        cls_preds = (
            output_dict[f"cls_preds{suffix}"]
            .permute(0, 2, 3, 1)
            .contiguous()
            .view(batch_size, -1, 1)
        )
        ...
        cls_loss = sigmoid_focal_loss(
            cls_preds, cls_labls, weights=cls_weights, **self.cls
        )
        ...
        reg_preds = (
            output_dict[f"reg_preds{suffix}"]
            .permute(0, 2, 3, 1)
            .contiguous()
            .view(batch_size, -1, 7)
        )
        ...
        reg_loss = weighted_smooth_l1_loss(
            reg_preds, reg_targets, weights=reg_weights, sigma=self.reg["sigma"]
        )
        ...
        if self.dir:
            dir_targets = self.get_direction_target(
                target_dict["targets"].view(batch_size, -1, 7)
            )
            dir_logits = (
                output_dict[f"dir_preds{suffix}"]
                .permute(0, 2, 3, 1)
                .contiguous()
                .view(batch_size, -1, 2)
            )  # [N, H*W*#anchor, 2]
            ...
            dir_loss = softmax_cross_entropy_with_logits(
                dir_logits.view(-1, self.anchor_num),
        ...
        if self.iou:
            iou_preds = (
                output_dict["iou_preds{suffix}"].permute(0, 2, 3, 1).contiguous()
            )
            ...
            boxes3d_tgt = VoxelPostprocessor.delta_to_boxes3d(
                target_dict["targets"], output_dict["anchor_box"]
            )[pos_pred_mask]
            iou_weights = reg_weights[pos_pred_mask].view(-1)
            iou_pos_targets = (
                aligned_boxes_iou3d_gpu(
                    boxes3d_pred.float()[:, [0, 1, 2, 5, 4, 3, 6]],  # hwl -> dx dy dz
                    boxes3d_tgt.float()[:, [0, 1, 2, 5, 4, 3, 6]],
                )
                .detach()
                .squeeze()
            )
            iou_pos_targets = 2 * iou_pos_targets.view(-1) - 1
            iou_loss = weighted_smooth_l1_loss(
                iou_pos_preds,
                iou_pos_targets,
                weights=iou_weights,
    ...
    def get_direction_target(self, reg_targets):
        ...
        anchor_yaw = np.deg2rad(
            np.array(self.dir["args"]["anchor_yaw"])
        ...
        anchor_map = self.anchor_yaw_map.repeat(
            1, H_times_W_times_anchor_num // self.anchor_num, 1
        ).to(reg_targets.device)  # [1, H * W * #anchor_num, 1]
        rot_gt = reg_targets[..., -1] + anchor_map[..., -1]  # [N, H*W*anchornum]
        offset_rot = limit_period(rot_gt - dir_offset, 0, 2 * np.pi)
        dir_cls_targets = torch.floor(
            offset_rot / (2 * np.pi / num_bins)
        ).long()  # [N, H*W*anchornum]
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None, suffix=""):
        ...
        if not writer is None:
            writer.add_scalar(
                "Regression_loss" + suffix, reg_loss, epoch * batch_len + batch_id
            )
            writer.add_scalar(
                "Confidence_loss" + suffix, cls_loss, epoch * batch_len + batch_id
            )
            writer.add_scalar(
                "Dir_loss" + suffix, dir_loss, epoch * batch_len + batch_id
            )
            writer.add_scalar(
                "Iou_loss" + suffix, iou_loss, epoch * batch_len + batch_id
            )


def one_hot_f(tensor, num_bins, dim=-1, on_value=1.0, dtype=torch.float32):
    tensor_onehot = torch.zeros(
        *list(tensor.shape), num_bins, dtype=dtype, device=tensor.device
    )
...
def sigmoid_focal_loss(preds, targets, weights=None, **kwargs):
    ...
    p_t = (targets * prediction_probabilities) + (
        (1 - targets) * (1 - prediction_probabilities)
    )
    modulating_factor = torch.pow(1.0 - p_t, kwargs["gamma"])
    alpha_weight_factor = targets * kwargs["alpha"] + (1 - targets) * (
        1 - kwargs["alpha"]
    )
