...
class PointPillarLossOpenCOOD(nn.Module):
    ...
    def forward(self, output_dict, target_dict, epoch=1, train=True):
        ...
        if "pred_feature" in output_dict.keys() and "rm" in output_dict.keys():
            ...
            diff_loss = self.diff_loss_func(
                p_e, g_e, weights=self.sigmoid_weight(3, epoch) if train else 0
            )  # self.sigmoid_weight(4, epoch)
            ...
            self.loss_dict.update(
                {
                    "total_loss": self.total_loss / self.loss_cnt,
            ...
            p_e = output_dict["pred_feature"]
            g_e = output_dict["gt_feature"]
            diff_loss = self.diff_loss_func(p_e, g_e, weights=1)
    ...
    def diff_loss_func(
        self, pred_noise: torch.Tensor, gt_noise: torch.Tensor, weights: int
    ):
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        total_loss = (
            ...
            if "total_loss" in self.loss_dict.keys()
            else torch.tensor(0)
        )
        reg_loss = (
            self.loss_dict["reg_loss"]
            if "reg_loss" in self.loss_dict.keys()
            else torch.tensor(0)
        )
        conf_loss = (
            self.loss_dict["conf_loss"]
            if "conf_loss" in self.loss_dict.keys()
            else torch.tensor(0)
        )
        if pbar is None:
            if "diff_loss" in self.loss_dict:
                ...
                print(
                    ...
                    " || Loc Loss: %.4f"
                    % (epoch, batch_id + 1, batch_len, total_loss, conf_loss, reg_loss)
                )
        else:
            if "diff_loss" in self.loss_dict:
                diff_loss = self.loss_dict["diff_loss"]
        ...
        writer.add_scalar("Confidence_loss", conf_loss, epoch * batch_len + batch_id)
