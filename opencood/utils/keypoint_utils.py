...
def get_keypoints(pcd_all_np, pcd_select_np, n_samples, mode="farthest"):
    if pcd_select_np.shape[0] >= n_samples:
        ...
        keypoints = get_random_subset(
            pcd_all_np, n_samples - pcd_select_np.shape[0], mode
        )
...
def bev_sample(pcd_np, lidar_range, n_samples, mode, voxel_size=0.2, all_samples=False):
    ...
    print(3)
    points_select = get_point_in_voxels(
        pcd_np, rows, cols, lidar_range, voxel_size, pcd_with_idx
    )
    print(4)
...
def seq_generate():
    dirs = [
        "/GPFS/rhome/yifanlu/workspace/dataset/OPV2V/train",
        "/GPFS/rhome/yifanlu/workspace/dataset/OPV2V/validate"
        "/GPFS/rhome/yifanlu/workspace/dataset/OPV2V/test",
    ]

    kp_store_path = (
        "/GPFS/rhome/yifanlu/workspace/OpenCOOD/keypoints_file/bev_keypoints"
    )
...
def parallel_generate(scenario_folder, scenario_folder_name):
    ...
    for j, cav_id in enumerate(cav_list):
        ...
        for timestamp in timestamps:
            ...
            kp_paths = [
                f"{kp_store_path}/{scenario_folder_name}/{cav_id}/{timestamp}.npy"
            ]
            kp_paths += [
                f"{kp_store_path}_{n_samples}/{scenario_folder_name}/{cav_id}/{timestamp}.npy"
            ...
            all_keypoint = bev_sample(
                pcd_np, lidar_range, np.inf, mode="uniform", all_samples=True
            )
...
if __name__ == "__main__":
    ...
    scenario_folders = [
        "/GPFS/rhome/yifanlu/workspace/OpenCOOD/dataset_link/validate/2021_08_21_17_30_41"
    ]
    ...
    for i in range(num):
        p = Process(
            target=parallel_generate,
            args=(scenario_folders[i], scenario_folders_name[i]),
        )
        p.start()
