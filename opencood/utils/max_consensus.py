...
def max_consunsus_hierarchical(
    ...
    **kwargs,
    ...
    pointsl_out, pointsr_out, T, tf_local, cons, matched_pointsl, matched_pointsr = (
        max_consensus2(
            pointsl,
            pointsr,
            -max_err,
...
def max_consensus2(
    ...
    tf_matrices, tf_params, tf_params_local = construct_tfs(
        xyr_min, xyr_max, resolotion, loc_l, loc_r
    )
    ...
    pointr_transformed = np.einsum(
        "...ij, ...jk", tf_matrices, np.tile(pointr_homo, (len(tf_matrices), 1, 1))
    ).transpose(0, 2, 1)
    ...
    for R in rotl[:, :2, :2]:
        ...
        nbrs = NearestNeighbors(n_neighbors=1, radius=radius, algorithm="auto").fit(
            pointl_transformed
        )
        distances, indices = nbrs.kneighbors(pointr_transformed_s)
        mask = distances < radius
        lbll, lblr = point_labels
        plus = (np.logical_and(lbll[indices] > 2, mask)).reshape(
            len(tf_matrices), len(pointsr)
        )
        ...
        if match_consensus > cur_cons:
            ...
            selected_indices = indices.reshape(len(tf_matrices), len(pointsr))[
                best_match
            ][accurate_points_mask]
...
def max_consensus1(
    ...
    match_tf_local = tf_params_local[best_match]
    accurate_points_mask = plus[best_match]
    selected_indices = indices.reshape(len(tf_matrices), len(pointsr))[best_match][
        accurate_points_mask
    ]
    matched_pointsl = pointsl[selected_indices]
    matched_pointsr = pointsr[accurate_points_mask]
...
def construct_tfs(xyr_min, xyr_max, resolution, loc_l=None, loc_r=None):
    input = [
        np.arange(xyr_min[i], xyr_max[i], resolution[i]) for i in range(len(xyr_min))
    ]
    ...
    tfs = np.array([[cosa, -sina, x], [sina, cosa, y], [zeros, zeros, ones]]).transpose(
        2, 0, 1
    )
