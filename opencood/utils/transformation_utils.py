...
__all__ = [
    ...
    "muilt_coord",
    "veh_side_rot_and_trans_to_trasnformation_matrix",
    "normalize_pairwise_tfm",
    "inf_side_rot_and_trans_to_trasnformation_matrix",
    "rot_and_trans_to_trasnformation_matrix",
]
...
def tfm_to_pose(tfm: np.ndarray):
    ...
    roll = np.degrees(
        np.arctan2(-tfm[2, 1], tfm[2, 2])
    )  # but counter-clockwise in carla
    pitch = np.degrees(
        np.arctan2(tfm[2, 0], ((tfm[2, 1] ** 2 + tfm[2, 2] ** 2) ** 0.5))
    )  # but counter-clockwise in carla
...
def tfm_to_pose_torch(tfm: torch.Tensor, dof: int):
    ...
    roll = torch.rad2deg(
        torch.atan2(-tfm[:, 2, 1], tfm[:, 2, 2])
    )  # but counter-clockwise in carla
    pitch = torch.rad2deg(
        torch.atan2(tfm[:, 2, 0], (tfm[:, 2, 1] ** 2 + tfm[:, 2, 2] ** 2) ** 0.5)
    )  # but counter-clockwise in carla
...
def get_pairwise_transformation_torch(lidar_poses, max_cav, record_len, dof):
    ...
    pairwise_t_matrix = (
        torch.eye(4, device=lidar_poses.device)
        .view(1, 1, 1, 4, 4)
        .repeat(B, max_cav, max_cav, 1, 1)
    )  # (B, L, L, 4, 4)
...
def normalize_pairwise_tfm(pairwise_t_matrix, H, W, discrete_ratio, downsample_rate=1):
    ...
    pairwise_t_matrix[..., 0, 2] = (
        pairwise_t_matrix[..., 0, 2] / (downsample_rate * discrete_ratio * W) * 2
    )
    pairwise_t_matrix[..., 1, 2] = (
        pairwise_t_matrix[..., 1, 2] / (downsample_rate * discrete_ratio * H) * 2
    )
...
def veh_side_rot_and_trans_to_trasnformation_matrix(
    ...
    rotation, translation = muilt_coord(
...
def inf_side_rot_and_trans_to_trasnformation_matrix(json_file, system_error_offset):
    ...
    translation[1][0] = (
        translation[1][0] + system_error_offset["delta_y"]
...
def relative_tfm_x1_to_x2(x1_rel, x1_center, x2_rel, x2_center):
    ...
    T_x1_rel2world = T_x1_center2world @ T_x1_rel2center

    x1_world_pose = tfm_to_pose(T_x1_rel2world)

    T_x2_rel2center = pose_to_tfm(np.array([x2_rel]))[0]
    T_x2_center2world = pose_to_tfm(np.array([x2_center]))[0]
    T_x2_rel2world = T_x2_center2world @ T_x2_rel2center

    x2_world_pose = tfm_to_pose(T_x2_rel2world)

    T_x1_to_x2 = x1_to_x2(x1_world_pose, x2_world_pose)
    return T_x1_to_x2
