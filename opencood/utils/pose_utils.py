...
def add_noise_data_dict(data_dict, noise_setting):
    ...
    if noise_setting["add_noise"]:
        for cav_id, cav_content in data_dict.items():
            cav_content["params"]["lidar_pose_clean"] = cav_content["params"][
                "lidar_pose"
            ]  # 6 dof pose

            if (
                "laplace" in noise_setting["args"].keys()
                and noise_setting["args"]["laplace"] is True
            ):
                cav_content["params"]["lidar_pose"] = cav_content["params"][
                    "lidar_pose"
...
def generate_noise_torch(pose, pos_std, rot_std, pos_mean=0, rot_mean=0):
    ...
    noise[:, 4] = (
        dist.von_mises.VonMises(loc=rot_mean, concentration=concentration)
        .sample((N,))
        .to(noise.device)
    )
