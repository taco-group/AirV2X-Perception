...
def logbessel_I_scipy(nu, z, check=True):
    ...
    if check:
        assert len(result[result == 0]) == 0, (
            "Bessel functions take " + "value 0 for z = {}".format(z[result == 0])
        )
...
def logbessel_I_asymptotic(nu, z):
    ...
    result[z <= nu] = (
        nu * torch.log(z[z <= nu])
        + eta * z[z <= nu]
        - (nu + eta) * torch.log(torch.tensor(2.0))
        - torch.log(torch.tensor(special.gamma(nu + 1)))
    )

    result[z > nu] = (
        z[z > nu] - 0.5 * torch.log(z[z > nu]) - 0.5 * torch.log(torch.tensor(2 * pi))
    )
    return result
...
def logbessel_I(nu, z, fast=False, check=True):
    ...
    if check:
        ...
        # logbessel_approx
        assert len(result[torch.isnan(result)]) == 0, (
            "Bessel functions take "
            + "NaN value for z = {}".format(z[torch.isnan(result)])
        )
        assert len(result[torch.isinf(result)]) == 0, (
            "Bessel functions take "
            + "inf value for z = {}".format(z[torch.isinf(result)])
        )
    return result
...
if __name__ == "__main__":
    ...
    plt.plot(
        z,
        torch.abs((approx - scipy_adapt) / scipy_adapt),
        "-",
        label=None,
        linewidth=linewidth,
    )
